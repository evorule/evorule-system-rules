...
def main():
    ...
    # --- v1.0 固化专项（对照 TCB 源码：set.value / merge tool_results / __ 前缀）---
    def rs(transform):
        return {
            "$schema": "https://evorule.org/schemas/rule_set/v1.0.json",
    ...
    # merge 用 tool_results（复数，多工具扇出）→ 引擎合法（exec_merge 优先读 tool_results）→ 应通过
    all_ok &= case(rs([{"type": "merge", "params": {
        ...
        rs_schema, registry, "正向：merge 用 tool_results 复数（引擎合法）→ 期望通过", True)

    # merge 既无 tool_result 也无 tool_results → 引擎 MissingField → 应被拒
    all_ok &= case(rs([{"type": "merge", "params": {
        "messages": "__exec__.payload.llm_response.messages",
        "next_instruction": {"type": "noop"}}}]),
        rs_schema, registry, "负向：merge 无 tool_result 且无 tool_results → 期望被拒", False)
    ...
    # collect 带 after（引擎 exec_collect 末尾支持）→ 应通过
    all_ok &= case(rs([{"type": "collect", "params": {
        "from": "__exec__.payload.llm_response.tool_calls",
        "each": {"type": "noop"},
        "after": {"type": "noop"}}}]),
        rs_schema, registry, "正向：collect 带 after 可选参数 → 期望通过", True)
